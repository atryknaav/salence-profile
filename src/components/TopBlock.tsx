...
const TopBlock = (props: { name: BlockName, status: string, weekPercent: string, weekNum: string  }) => { 
    ...
    const styleBody = name === currBlock
        ...
        : ` transition-colors hover:bg-success hover:cursor-pointer ${isDark 
  ...
  return (
    ...
    <div className={'flex flex-col group flex-1 first:rounded-l-lg last:rounded-r-lg p-3 mt-20 lg:mt-0' + styleBody} 
        ...
        <div className="flex gap-2">
            <div className={`rounded-full p-1 h-[fit-content] bg-black ${name === currBlock 
                ? 'bg-black text-success' 
                : `bg-black group-hover:bg-black group-hover:text-success ${isDark
                ?   'bg-light-gray'
                :   'bg-black text-white'}`}`}>
                {icon}
            </div>

            <div className={`text-md mb-6 ${name === currBlock 
                ? 'text-black' 
        ...
        <div className={`mb-3 font-semibold text-3xl ${name === currBlock ? 'text-black' : `group-hover:text-black ${isDark
            ? 'text-white'
            : 'text-black'}`}`}>
            {`${status}`}
        </div>
        ...
        <div className='flex gap-3 text-sm font-medium mb-1'>
            ...
            <div className={`flex items-center gap-2 ${name === currBlock ? 'text-minor-success' : ` group-hover:text-minor-success ${isDark
                ? 'text-success'
                : 'text-upgrade'}`}`}>
                {arrowUp} {`${weekPercent}%`}
            </div>
